...
The fidelity of the generated images was assessed using three independent classifiers, one for each demographic attribute [[1]](#references). A synthetic image is considered successfully generated only if it satisfies two conditions: (1) the target intervention is correctly predicted, and (2) all non-intervened attributes remain consistent with the original image. If any attribute fails this check, the generation is retried up to three times. If the prediction fails after three attempts, the prompt is discarded. This validation process is referred to as the *Attribute Check* module. Consequently, the final dataset size depends on the acceptance rate of this validation step, with a theoretical maximum of 5,000 images (5 groups × 1k samples). We evaluated this pipeline across three generative architectures: a Variational Autoencoder (VAE) producing counterfactuals [[2]](#references), a diffusion model fine-tuned on MIMIC-CXR (Roentgenv2 [[3]](#references)), and a DDIM inversion scheme applied to the counterfactual version of the latter [[4]](#references). An overview of the pipeline used in this work can be seen below.
